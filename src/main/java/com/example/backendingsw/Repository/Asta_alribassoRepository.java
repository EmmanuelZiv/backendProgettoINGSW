...
public interface Asta_alribassoRepository extends JpaRepository<Asta_alribasso, Long> {
    ...
    @Query("SELECT a FROM Asta_alribasso  a JOIN AsteCategorieAlRibasso ac ON a.id = ac.asta.id WHERE a.condizione = 'aperta' AND ac.nomeCategoria = :nomeCategoria ORDER BY a.intervalloDecrementale ASC LIMIT 15")
    List<Asta_alribasso> findFirst15ByCategorieNomeAndCondizioneAperta(String nomeCategoria);
