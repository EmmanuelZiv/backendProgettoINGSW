...
public interface Asta_inversaRepository extends JpaRepository<Asta_inversa, Long> {
    ...
    List<Asta_inversa> findFirst15ByCondizioneOrderByDataDiScadenzaAsc(String condizione);

    List<Asta_inversa> findFirst15ByCondizioneOrderByIdDesc(String condizione);

    @Query("SELECT a FROM Asta_inversa a JOIN AsteCategorieInversa ac ON a.id = ac.asta.id WHERE a.condizione = 'aperta' AND ac.nomeCategoria = :nomeCategoria ORDER BY a.dataDiScadenza ASC LIMIT 15")
    List<Asta_inversa> findFirst15ByCategorieNomeAndCondizioneAperta(String nomeCategoria);
